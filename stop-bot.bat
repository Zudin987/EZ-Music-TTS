...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$expected=Join-Path $env:EZ_MUSIC_ROOT 'src\index.js'; $pidFile=$env:BOT_PID_FILE; $matches={param($p) $p -and ([string]$p.Name -match '^node(\.exe)?$') -and ([string]$p.CommandLine).IndexOf($expected,[StringComparison]::OrdinalIgnoreCase) -ge 0}; $p=$null; if(Test-Path -LiteralPath $pidFile){$raw=(Get-Content -LiteralPath $pidFile -Raw -ErrorAction SilentlyContinue).Trim(); if($raw -match '^\d+$'){$candidate=Get-CimInstance Win32_Process -Filter ('ProcessId='+[int]$raw) -ErrorAction SilentlyContinue; if(& $matches $candidate){$p=$candidate}}}; if(-not $p){$p=Get-CimInstance Win32_Process -Filter 'Name=''node.exe''' -ErrorAction SilentlyContinue | Where-Object {& $matches $_} | Select-Object -First 1}; if(-not $p){exit 2}; Stop-Process -Id $p.ProcessId -Force -ErrorAction Stop; exit 0"
